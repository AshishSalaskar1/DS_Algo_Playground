...
2. We want to replace arr[i] such that its only little greater than the current number. 
   So replace it with lowest ele greater than arr[i] in the right side of the array. 
   (Why smallest + greater?  we wnt next consecutive perm not largest)
3. Now array till arr[i] is already greater than arr. Now remaining element on the right side needs to be smallest as possible 
   (Because the arr[:i] is already greater than arr). 
   So reverse the right side arr (or sort acc in ascending order)

...
//		parent[src] = -1;
		...
		for(int j=0;j<V-1;j++) {
			int minNode = findMinNode(vis, weight);
			vis[minNode] = true;
			
			for(int i=0;i<V;i++) {
				if(adj[minNode][i]!=0 && !vis[i]) {
					
					if(adj[minNode][i] < weight[i])
					{
						weight[i] = adj[minNode][i];
						parent[i] = minNode;
					}
				}
			}
		...
		System.out.println("The edges included in MST are: ");
		int cost = 0;
		...
		System.out.println("The cost of MST: "+cost);
		System.out.println();

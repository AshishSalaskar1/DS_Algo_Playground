...
class BST{
	...
	int findNearestInBST(Node node,int val,int closestSF) {
		...
		if(val < node.data)
			return findNearestInBST(node.left,val,closestSF);
		else 
			return findNearestInBST(node.right,val,closestSF);
	...
	public static void main (String[] args) throws Exception
	 {
		...
		System.out.println(t.findNearestInBST(t.head,25,Integer.MAX_VALUE));
